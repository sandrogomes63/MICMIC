...
unsigned char velocidade, tempVelocidade, tempLeitura = 50;
...
void init(void)
{
	...
	// ADC
	ADMUX = 0b00100000;	 // AREF, ajustado direita, canal 2
...
char ler_AD(void)
{ // leitura do valor de ADC
	unsigned char leituraH;

	ADCSRA = ADCSRA | 0b01000000; // ADSC = 1

	while ((ADCSRA & (1 << ADSC)) != 0); // espera ADSC = 0
...
unsigned char Vin(void)
{ // função para calculo de Vin 
...
void switch_mode(void)
{
	...
	case 0b00110010: // sw1
		if (noSw == 0)
		{
			noSw = 1;
			sw6 = 0;
			if (velocidade < 96) // limita velocidade a 100%
			{
				sw6 = 0;
	...
	if (rxUsart.receive == 1) // verificar se novos dados foram recebidos
	{
		...
		else
		{
			*buffer = rxUsart.receiver_buffer;
			...
			case 'A':
			case 'a':
				sprintf(transmit_buffer, "Tecla: A \r\n");
				send_message(transmit_buffer);
				mode = 'A';
...
//modo controlo por analogico pontenciometro 
void analogic_mode(void)
{
	unsigned char switches = 0;
	unsigned char tempLeitura[2];
	switches = PINA & 0b00110011; // verifica apenas os switches que pretendemos que altere o funcionamento
	int i = 0;
	while (i < 2)
	{
	...
	velocidade = (tempLeitura[0] + tempLeitura[1]) / 2;
...
int main(void)
{
	...
	while (1)
	{
		...
		if (flag5ms == 1)				//verifica se ja passou 5ms
		{
			flag5ms = 0;
			updateDisplay();
			motor();
...
// interrupção de receção de uma trama
ISR(USART1_RX_vect)
{
